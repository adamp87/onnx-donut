...
class OnnxPredictorBase:
    ...
    def generate(self, img, prompt, max_length=None):
        ...
        out_encoder = self._run_encoder({'pixel_values': encoder_input_ids})
        ...
        while not stop:
            if past_key_values is None:
                logits, past_key_values = self._run_decoder({'input_ids': input_ids, 'encoder_hidden_states': out_encoder})

            else:
                logits, past_key_values = self._run_decoder_with_past({'input_ids': input_ids[:, -1:],
                                                           **past_key_values})
            ...
            if end_token is None:
                ...
                if content is not None:
                    ...
                    if r"<s_" in content and r"</s_" in content:  # non-leaf node
                        value = OnnxPredictorBase.token2json(content, tokenizer, is_inner_value=True)
                        if value:
                            if len(value) == 1:
                                value = value[0]
                            output[key] = value
                ...
                if tokens[:6] == r"<sep/>":  # non-leaf nodes
                    return [output] + OnnxPredictorBase.token2json(tokens[6:], tokenizer, is_inner_value=True)
